...
if (process.env.NODE_ENV !== "production") {
  if (!isLiveUrl(APP_STORE_URL) || !isLiveUrl(GOOGLE_PLAY_URL)) {
    console.warn(
      '[NavStore] Store URL is still a placeholder ("#") — that button is hidden until a real URL is set.',
    );
  }
}
...
export default function NavStore() {
  ...
  // Nothing to link to yet — render nothing rather than an orphaned "Get the
  // app" label with no buttons.
